...
class FetchJeepTest {
	...
	class TestsThatDoNotPolluteTheApplicationContext {
		...
		@Test
		void testThatAnErrorMessagisReturnedWhenAnUnknownTrimIsSupplied() {
		...
		void testThatAnErrorMessagisReturnedWhenAnInvalidValueIsSupplied(String model, String trim, String reason) {
			// Given an invalid trim...
			String uri = String.format("http://localhost:%d/jeeps?model=%s&trim=%s",
					serverPort, model, trim);
			...
			// Then a not found (404) status code is returned...
			assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST); // error code 400
